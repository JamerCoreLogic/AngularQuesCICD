  ...
  <div class="qlist-table-header">
    ...
    <div  class="qth-right">
      ...
      <span *ngIf="AdvanceFilterEnabled" class="advancefilterbtn" (click)="OpenFilter()">Advanced Filter</span>
          ...
          <select class="form-control dropdown_border" [(ngModel)]="selectedAgent"  (change)="selectAgent($event.target.value)" >
            <option value='0' selected>Select</option>
            <option *ngFor="let agent of AgentList" [value]="agent.agentId">{{agent.agentName}}</option>
          </select>
        </div>
        <button *ngIf="IsCheckboxEnable" [ngClass]="{disabledButton:!isValidTransfer}" [disabled]="!isValidTransfer" type="submit" (click)="transferBusiness()" class="btn-theme fix_width" >
                        Transfer
        </button>
      </div>
