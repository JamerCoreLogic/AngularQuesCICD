...
@Component({
  ...
  <div class="qlist-table-header">
    <div class="qth-left">
      <span *ngIf="Pagination"><input type="text" class="greybg-input" placeholder="Search" [(ngModel)]="searchText" (keyup)="filter(searchText)" /></span>
    </div>
    <div  class="qth-right">
    <span *ngIf="AdvanceFilterEnabled" class="advancefilterbtn" (click)="clearAll()" style="margin-left:16% !important;">Clear Filter</span>        
      <span *ngIf="AdvanceFilterEnabled" class="advancefilterbtn" (click)="OpenFilter()">Advanced Filter</span> 
          <div *ngIf="IsCheckboxEnable" class="business-search" style="display:inline-block; width:431px; float:left; margin-left: 240px;">
            <div class="form-group" style="display: inline-block; width: 216px;float: left;margin-left: 45px;line-height: 8px; text-align:left; margin-top:7px;">
          <label class="textbox_borderlabel">Transfer to</label>
          <select class="form-control dropdown_border"  (change)="selectAgent($event.target.value)" style="height:32px; margin-top:4px; padding-top:5px;">
            <option value='0' [selected]="resetAgent">Select</option> 
            <option *ngFor="let agent of AgentList" [value]="agent.agentId">{{agent.agentName}}</option>
          </select>
        </div>
        <button *ngIf="IsCheckboxEnable" [disabled]="!isValidTransfer" type="submit" (click)="transferBusiness()" class="btn-theme fix_width" style="display: inline-block;float: left; margin-left: 35px; margin-top: 11px;">
                        Transfer
        </button>
      </div>    
  ...
  <table class="quote-table row-border hover">
    ...
    <ng-container *ngFor="let item of PageItems; let i = index">
      <tr class="main-row">
       <!-- <td>
          <span *ngIf="item.data" class="expandable-icon" (click)="ExpandTable(i)">
            <img [src]="ExpandedRowIndex != i? 'assets/images/tableplus.png' : 'assets/images/tableminus.png'"/>
          </span>
        </td>
        ...
        <td>
            ...
            <div class="styled-checkbox" *ngIf="IsCheckboxEnable && DataSource?.length > 0">
              <div class="checkboxgrid">
                  <!--<input type="checkbox" [id]="item.quoteId" [checked]="IsHeaderChkSelected" (change)="setHeaderCheckbox($event.target.checked,item.quoteId)">-->
      ...
      <tr class="table-row-expandable" *ngIf="ExpandedRowIndex == i">
        <td colspan="9">
          <table class="quote-table-inner">
            ...
            <ng-container *ngFor="let inneritem of item.data; let i = index">
              <tr *ngIf="inneritem['ExpandedRowIndex']">
  ...
  <ng-container *ngIf="Pagination">
    <lib-pagination [Pager]="pager" (OutputPage)="setPage($event)"></lib-pagination>
  </ng-container>         
  </div>        
</div>      
  `,
  ...
  setPage(page: number) {
    ...
    if (this.DataSource) {
      if (this.Pagination) {
        ...
        this.pager = this.pagerService.getPager(this.DataSource.length, page, this.pageItemCount);
