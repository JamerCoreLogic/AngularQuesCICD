...
import { Tasks } from 'src/app/models/user-models';
...
displayedColumns: string[] = [
  ...
  fieldDisplayNameMapping: { [key: string]: string } = {
    ...
    emailAddress: 'Email',
    userId: "User Id",
    backgroundCheckStatus: 'Background Status Check',
    fieldAdjusterContract: 'Field Adjuster Contract',
    deskAdjusterContract: 'Desk Adjuster Contract',
  ...
  @ViewChild(TaskfilterComponent) taskfilter!: TaskfilterComponent;
  loggedUserRes: any;
  loggedUserType: any;
  userType: any;
  ...
  operator: any;
   getTableData$(data:any) {
      return this.authService.GetTaskListForAdminDashboard(data)
    }
    public dateRange: {
      start: Date | null,
      end: Date | null
    ...
    primaryStatus: string = "primaryStatus = 'Pending'";
  ...
  getUserResources() {
    this.authService.getUsertaskSummary().subscribe((res: any) => {
      if (res != null) {
      ...
      .pipe(
        ...
        switchMap(() => {
          ...
          let payload = {
            ...
            filterValue: this.primaryStatus,
  ...
  prepareDataForDownload(data: Tasks[]) {
...
lockProfile(user: any) {
  ...
  pendingList() {
    this.isPending = true;
    this.primaryStatus = "primaryStatus = 'Pending'";
    this.searchString = '';
    this.filterBy='All';
    this.applyFilter()
  }
  expiredList() {
    ...
    this.primaryStatus = "primaryStatus = 'Expired'";
    this.searchString = '';
    this.filterBy='All';
    this.applyFilter()
