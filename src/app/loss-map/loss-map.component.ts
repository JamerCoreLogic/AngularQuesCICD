...
import { catchError, debounceTime, finalize, Observable, of, Subject, Subscription } from 'rxjs';
...
@Component({
  ...
  styleUrls: ['./loss-map.component.scss']
 
})
export class LossMapComponent implements OnInit {
  ...
  this.subscriptions.add(
    ...
    this.searchDebouncer.pipe(
      ...
      next: ({ searchText, radius, filterString ,companyNameFilter, surveyNameFilter,surveyTitleFilter, operator}) => {
        localStorage.setItem('lossMap',JSON.stringify({ searchText, radius, filterString,companyNameFilter,surveyNameFilter,surveyTitleFilter,operator}));
  ...
  savedFilterState(){
    const savedFilterState = localStorage.getItem('lossMap')
    if(savedFilterState){
      ...
      this.adjustersService.getAdjustersByAddress(payload).pipe(
        ...
        (response: any) => {
          ...
          if (response.data && response.data.list && response.data.list.length > 0) {
            ...
            // Use Google Maps Distance Matrix API to calculate driving distances
            this.spinner.show();
            
            // Calculate driving distances for all adjusters within the radius
            this.sharedAS.calculateBatchDrivingDistances(
              this.center.lat,
              this.center.lng,
              this.adjusters
            ).then(adjustersWithDrivingDistance => {
              // Filter adjusters based on driving distance being less than or equal to radius
              this.adjusters = adjustersWithDrivingDistance.filter((adjuster: any) => {
                const drivingDistanceValue = parseFloat(adjuster.drivingDistance?.replace(/,/g, '').replace(' mi', '') || '0');
                return drivingDistanceValue <= radius;
              });
              
              // console.log("adjustersWithDrivingDistance", this.adjusters);
              
              if (this.adjusters.length === 0) {
                Swal.fire({
                  title: 'No Adjusters Found',
                  text: 'No adjusters found within the specified driving distance radius.',
                  icon: 'warning',
                  confirmButtonText: 'Ok',
                  confirmButtonColor: '#ffa022',
                });
              ...
              // Sort the adjusters by distance and user type
              this.adjusters.sort((a: any, b: any) => {
                // Extract numeric values from drivingDistance strings and handle comma-separated numbers
                const distanceA = parseFloat(a.drivingDistance?.replace(/,/g, '').replace(' mi', '') || '0');
                const distanceB = parseFloat(b.drivingDistance?.replace(/,/g, '').replace(' mi', '') || '0');
              ...
              // console.log('Adjusters with driving distances:', this.adjusters);
              ...
              this.markerPositions = this.adjusters.map((adjuster:any) => ({
                ...
                drivingDistance: adjuster.drivingDistance || 0,
                drivingDuration: adjuster.drivingDuration || '',
              ...
              // Fall back to using the original distance calculations from the API
              this.adjusters = this.adjusters.filter((adjuster: any) => {
                const drivingDistanceValue = parseFloat(adjuster.drivingDistance?.replace(/,/g, '').replace(' mi', '') || '0');
                return drivingDistanceValue <= radius;
              });
              ...
              let allAdjusters = [...this.externalUserList, ...this.externalUserListFiletrac, ...this.interUserList];
              this.markerPositions = allAdjusters;
            ...
            if (response.message?.trim().toLowerCase() === 'invalid address'.toLowerCase()) {
              ...
              localStorage.removeItem('lossMap')
            } else {
              Swal.fire({
                title: 'No Adjusters Found',
                icon: 'warning',
