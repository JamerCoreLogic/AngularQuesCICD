...
@Component({
  selector: 'loss-radius-slider',
  templateUrl: './loss-radius-slider.component.html',
  styleUrls: ['./loss-radius-slider.component.scss']
})
export class LossRadiusSliderComponent implements OnInit {
  radius: number = 25;
...
radiusIncrease() {
  if (this.radius < 500) {  // Check if radius is below the upper limit
    this.radius = this.radius + 10;
    if (this.radius > 500) { // Ensure radius doesn't exceed the upper limit
      this.radius = 500;
    }
    this.radiusDisplay = this.radius.toFixed(0) + ' miles';
