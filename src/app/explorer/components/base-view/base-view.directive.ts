...
export class BaseView implements OnDestroy {
    ...
    public downloadableFiles: boolean = false;
    ...
    constructor(protected explorerService: ExplorerService, protected helperService: HelperService, @Inject(FILTER_STRING) private filter: BehaviorSubject<string>) {
        ...
        this.subs.add(
            this.explorerService.selectableItems.subscribe((items) => {
                // console.log('selectableItems', items)
                this.downloadableFiles = items ? items : false;
            })
    ...
    getContent(data: any): string {
        return this.helperService.getContent(data);
    }
