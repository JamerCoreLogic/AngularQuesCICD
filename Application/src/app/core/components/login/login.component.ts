...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (account) {
      console.log("Azure Account Info:", account);
      console.log("Username:", account.username); // email
      console.log("Name:", account.name);         // full name
      console.log("Home Account ID:", account.homeAccountId);
      console.log("Local Account ID:", account.localAccountId);
      this.savedUserName = account.username;
      this.savedAccount = account;
      this.savedPassword = account.homeAccountId;
      this.savedRemember = true;
    } else {
      console.warn("No account is currently active.");
    }
  ...
  // loginAzure() {
  //   const test = this.msalService.loginRedirect();
  ...
  resolveRoute() {
    this._activatedRoute.data.subscribe((response: any) => {
      if (response?.mgaConfigResolver) {
        ...
        this.favIcon = config?.aqFavIconURL
        if (this.favIcon) {
  ...
  createLoginForm() {
    this.loginForm = this.fb.group({
      ...
      // password: [this.savedAccount.homeAccountId, [Validators.required, NoWhiteSpace]],
  ...
  onSubmit() {
    ...
    this._loginSubscription = this.login.LoginAuthentication(this.loginForm.value.userName, this._encryptionService.Encrypt(this.loginForm.value.password), this.rememberMe.value).subscribe(data => {
      ...
      // this._loginSubscription = this.login.LoginAuthentication(this.loginForm.value.userName, this.savedAccount.homeAccountId, this.rememberMe.value).subscribe(data => {
      this._loaderService.hide();
      const resp: any = data;
