...
export class HttpInterceptorService implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (req.body) {
      if ('ClientID' in req.body) {
        const updatedBody = { ...req.body, ClientID: Client['ClientID'] };
        cloned = req.clone({ body: updatedBody, headers });
      } else {
        cloned = req.clone({ body: { ...req.body, ...Client }, headers });
      }
    } else {
  ...
  modifyURL(req: HttpRequest<any>): string {
    if (req.url.includes('api/user/authenticateuser')) {
      return "http://10.130.205.69:9080/api/user/authenticateuser";
