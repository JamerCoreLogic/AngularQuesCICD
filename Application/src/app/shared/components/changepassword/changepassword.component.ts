...
export class ChangepasswordComponent implements OnInit, OnDestroy {
  ...
  changePassword(): void {
    ...
    const currentPassEncrypted = this._encryptionService.Encrypt(this.changeForm.value['currentPass']);
    const newPassEncrypted = this._encryptionService.Encrypt(this.changeForm.value['newPass']);
    const username = this._user.UserName();

    this._changePasswordService.ChangePassword(username, currentPassEncrypted, newPassEncrypted)
      .subscribe({
        next: (data) => {
          this.resetForm();
          const message = data?.message || 'Unknown response';
          const dialog = this._popup.showPopup('Change Password', message);
        ...
        error: (err) => {
          ...
          this._popup.showPopup('Change Password', 'An error occurred while changing the password.');
        },
        complete: () => {
          this._loader.hide();
        }
