...
export class QuotesChartComponent implements OnInit, AfterViewInit {
  ...
  getkpi(period: string, startDate: Date, endDate: Date) {
    this.ChartsData = [];
    if (this.agent?.Agent()?.agentId) {
      this._apki.AqkpiList(this._user.UserId(), period, startDate, endDate, 0, this.agent.Agent().agentId, "").subscribe(data => {
        if (data?.data?.charts) {
          const charts = data.data.charts[0]
          let seriesForNb = charts.nbPremiumsChart;
          let seriesForRN = charts.rnPremiumsChart;
          let seriesForTI = charts.totalIndications;
          let seriesforquotes = charts.totalQuotes;
