...
import { loadKpi } from 'store/actions/agent-dashboard.action';
import { selectAllKpiResponses, selectKpiNoRecordMessage } from 'store/selectors/agent-dashboard.selectors';
import { selectWorkPeriodList } from 'store/selectors/workPeriodType.selector';
import { loadPeriodTypes } from 'store/actions/workPeriodType.actions';
...
export class HeaderbarComponent implements OnInit {
  ...
  allKpis$: Observable<any[]>;
  noRecordsMessage$: Observable<string>;
  WorkPeriodDataSource$: Observable<any[]>;
  private WorkboardPeriodListSubscription: Subscription;
  ...
  displayTempData(data) {
    ...
    this.resp = this.resp?.map(data => {
      let performanceMeasureNameStr;
      const performanceMeasureName = data['performanceMeasureName'].toString().split(" ");

      if (performanceMeasureName.length > 1) {
        performanceMeasureNameStr = performanceMeasureName.pop();
      }

      return {
        ...data, // spread original
        performanceMeasureName: performanceMeasureName.join(" "),
        performanceMeasureNameStr
      };
    });
  ...
  // getPeriodType() {
  //   // this.WorkPeriodDataSource$ = this.store.select(selectWorkPeriodList);
  //   // this.WorkboardPeriodListSubscription?.unsubscribe();
  //   this.workboardService.workboardPeriodList().subscribe(data => {
  ...
  getPeriodType() {
    ...
    this.WorkboardPeriodListSubscription?.unsubscribe();
    // this.workboardService.workboardPeriodList().subscribe(data => {
    this.WorkboardPeriodListSubscription = this.WorkPeriodDataSource$.pipe().subscribe((data: any) => {
      if (!data || !data.success) {
        this.store.dispatch(loadPeriodTypes());
      }
      if (data?.data?.workboardResponse) {
        setTimeout(() => {
          this.workboardResponse = data.data.workboardResponse;
  ...
  showPeriodData(selectedPeriod: string) {
  ...
  getkpi(period: string) {
    const startDate = this.selectedPeriodText.startDate;
    const endDate = this.selectedPeriodText.endDate;
    const agentId = this.agent.Agent()?.agentId;
    ...
    this.kpiData$ = this.store.select(selectAllKpiResponses);
    this.noRecordsMessage$ = this.store.select(selectKpiNoRecordMessage);

    this.kpiData$.subscribe((kpis: any) => {
      ...
      //   status: 'DataUpdated'
      // });
      this.resp = kpis?.data?.kpiResponses?.filter(data => !data['aqkpi']).slice(0, 8);
