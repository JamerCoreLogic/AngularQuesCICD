...
export class QuotesviewComponent implements OnInit {
  ...
  setQuoteViewValue(quotedata: any): void {
    const formattedPhone = quotedata.phone
      ? quotedata.phone.replace(/^(\d{3})(\d{3})(\d{4})/, '($1) $2-$3')
      : '';

    const formattedEffectiveDate = this._datePipe.transform(quotedata.effective_Date, 'MM/dd/yyyy');
    const formattedExpiryDate = this._datePipe.transform(quotedata.expiry_Date, 'MM/dd/yyyy');

    const hasAssistants = Array.isArray(quotedata.underwriterAssistants) && quotedata.underwriterAssistants.length > 0;
    const assistantId = hasAssistants ? quotedata.underwriterAssistantID : null;
    ...
    const pendingElement = document.getElementById('Pending') as HTMLInputElement;
    if (pendingElement) {
      pendingElement.checked = true;
    }

    if (this.butDisabled) {
      this.quoteviewform.patchValue({
        SelectUnderwriter: quotedata.underwriterID,
        UwAssistent: assistantId,
        DispositionDecline: false
      });
  ...
  saveData(quoteviewform: any): boolean | void {
    const {
      DispositionAccept,
      DispositionDecline,
      SelectUnderwriter,
    ...
    let disposition = 'Refer To';
    let UW = 0;
    let UWA = 0;

    // Handle disposition states
    if (DispositionAccept) {
      disposition = 'Accept';
    } else if (DispositionDecline) {
      disposition = 'Decline';
      if (!comments?.trim()) {
        this.flag = true;
        return false;
      ...
      if (SelectUnderwriter && SelectUnderwriter !== 'null') {
        UW = SelectUnderwriter;
      } else if (UwAssistent && UwAssistent !== 'null') {
        UWA = UwAssistent;
        UW = UwID || 0;
      } else {
        UW = UwID || 0;
    ...
    this._quotesService.QuoteViewSubmition(this._quoteId, this._userId, disposition, UW, UWA, comments)
      .subscribe({
        next: (data) => {
          this.loaderService.hide();
          this.dataSource = data.success;
