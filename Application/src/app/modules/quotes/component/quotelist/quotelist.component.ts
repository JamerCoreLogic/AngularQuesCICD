...
import { debounceTime, map, distinctUntilChanged, finalize, take } from 'rxjs/operators';
...
import { selectMgaPrograms } from 'store/selectors/mgaProgram.selectors';
import { loadMGAPrograms } from 'store/actions/mgaProgram.action';
...
import { selectNoRecordsMessage, selectQuotes, selectQuotesLoading, selectQuotesTotal } from 'store/selectors/workboard.selectors';
import { loadQuotes } from 'store/actions/workboard.action';
...
export class QuotelistComponent implements OnInit, OnDestroy {
  mgaProgramDataSource$: Observable<any[]>;
  private MgaProgramListSubscription: Subscription;
  ...
  ngOnInit() {
    ...
    this._checkRoleService.addNewQuoteSubject
      ...
      .subscribe(value => {
        if (value === 'AddQuote') {
          const workboardStatus = this.workboardStatus ? `${this.workboardStatus} ${this.statusType}` : '';
          let request: IQuoteViewReq;

          if (this.advancedFilterRequest) {
            request = { ...this.advancedFilterRequest };
            this.assignCommonRequestParams(request);
          } else {
            request = {
              ...
              EffectiveFromDate: this.effectiveFromDate === '' ? null : this.effectiveFromDate,
              EffectiveToDate: this.effectiveToDate === '' ? null : this.effectiveToDate,
  ...
  seprateDate(date: string | Date): string | null {
    if (!date) {
      return null;
    }
  ...
  //   this.loaderService.show();
  //   this._quotesService.QuotesViewList(request, this.enabledForkJoin).pipe(
  //     finalize(() => this.loaderService.hide())
  //   ).subscribe(resp => {
  //     this.HideAdvFilterOption = true;
  //     if (resp[0]?.data?.quote) {
  //       setTimeout(() => this.dataSource = resp[0].data.quote, 100);
  //       if (this.enabledForkJoin && resp[1]) {
  //         this.totalItem = resp[1].totalQuote;
  //       }
  //       this.NoRecordsMessage = '';
  ...
  quoteViewListForMyDiary() {
    ...
    this._quotesService.QuotesViewList(request, this.enabledForkJoin).pipe(
      ...
      // this._checkRoleService.fromDiarySubject.next('');
      if (resp[0]?.data?.quote) {
        setTimeout(() => this.dataSource = resp[0]?.data?.quote, 100);
  ...
  private assignCommonRequestParams(request: IQuoteViewReq) {
    request.PageNumber = 1;
    request.PageSize = this.PageSize;
    request.SearchText = this.SearchText;
    request.SortingColumn = this.SortingColumn;
    request.SortingOrder = this.SortingOrder;
    request.SearchType = this.SearchType;
    request.CarrierID = request.CarrierID || 0;
    request.PolicyStartDateFrom = this.seprateDate(request.PolicyStartDateFrom);
    request.PolicyStartDateTo = this.seprateDate(request.PolicyStartDateTo);
    request.PolicyExpiryDateFrom = this.seprateDate(request.PolicyExpiryDateFrom);
    request.PolicyExpiryDateTo = this.seprateDate(request.PolicyExpiryDateTo);
  ...
  getParameterList() {
    ...
    //this.saveFilterService.GetAdvanceFilterParameter(AdvanceFilterType.quotesFilter, this._userId.toString(), this._agentId).subscribe(parameters => {
  ...
  getPeriod() {
    ...
    this.periodSubscription = this.period.period.subscribe(period => {
      ...
      if (period) {
        this._checkRoleService.fromDiarySubject
          .pipe(take(1)).subscribe((data) => {
            if (data === 'fromDiary') {
              if (quoteId) {
                ...
                this.workboardStatus = this.workboardStatus;
  ...
  filterQuotes(value: string) {
    if (value == 'Clear') {
      ...
      this.advancedFilterRequest = {
        ...
        PremiumStart: value['PremiumStart'] === '' ? null : parseInt(value['PremiumStart'], 10),
        PremiumEnd: value['PremiumEnd'] === '' ? null : parseInt(value['PremiumEnd'], 10),
  ...
  getMGAPrograms() {
    this.mgaProgramDataSource$ = this.store.select(selectMgaPrograms);
    this.MgaProgramListSubscription?.unsubscribe();
    // this._programService.MGAPrograms(this._userId, 1).subscribe(programs => {
    this.MgaProgramListSubscription = this.mgaProgramDataSource$.pipe().subscribe((data: any) => {
      if (!data || !data.success) {
        this.store.dispatch(loadMGAPrograms({ userId: this._userId, agencyId: 1 }));
      }
      if (data || data.success) {
        this.programData = data?.data?.mgaProgramList;
      }
      /* if (programs && programs.data && programs.data.programLob) {
        this.programData = programs.data.programLob.map(program => {
          return {
